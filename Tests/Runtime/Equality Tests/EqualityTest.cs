...
namespace GameJolt.NET.Tests.Equality
{
	public class EqualityTest
	{
		protected static void TestEquality<T>(Func<T, T, bool> compare, Func<T, T, bool> invertCompare, params T[] values) where T : struct, IEquatable<T>
		{
			...
			Assert.That(a.Equals(bObj), Is.True, "First value should be equal to itself in object form.");
			...
			for (int i = 1; i < values.Length; i++)
			{
				...
				Assert.That(a.Equals(bObj), Is.False, "First value should not be equal to the second value in object form.");
