...
namespace GameJolt.NET.Tests
{
	public class FriendsTest : BaseTest
	{
		...
		public async Task Fetch_Authenticated_Success()
		{
			...
			{
				return Task.FromResult(serializer.Serialize(new FetchFriendsResponse(true, null, new[]
				{
					new FriendId(0),
					new FriendId(1)
				})));
		...
		public async Task Fetch_NotAuthenticated_Fail()
		{
			GameJoltAPI.webClient.GetStringAsync("", default).ReturnsForAnyArgs(_ =>
			{
				return Task.FromResult(serializer.Serialize(new FetchFriendsResponse(false, null, new[]
				{
					new FriendId(0),
					new FriendId(1)
				})));
