...
namespace GameJolt.NET.Tests
{
	partial class DataStoreTest
	{
		...
		public async Task GetKeysUser_Buffer_Authenticated_Success(string pattern)
		{
			...
			// Act
			GameJoltResult result = await GameJoltAPI.DataStore.GetKeysAsCurrentUserAsync(buffer, pattern);

			// Assert
			Assert.That(result.HasError, Is.False);
			Assert.That(result.Exception, Is.Null);
			Assert.That(buffer, Has.Count.EqualTo(2));
			Assert.That(buffer[0], Is.EqualTo("key1"));
			Assert.That(buffer[1], Is.EqualTo("key2"));
		}
		...
		public async Task GetKeysUser_Buffer_Authenticated_Success_NoKeys()
		{
			...
			// Act
			GameJoltResult result = await GameJoltAPI.DataStore.GetKeysAsCurrentUserAsync(buffer);
		...
		[Test]
		public async Task GetKeysUser_Buffer_NotAuthenticated_Fail()
		{
			// Arrange
			List<string> buffer = new List<string>();
